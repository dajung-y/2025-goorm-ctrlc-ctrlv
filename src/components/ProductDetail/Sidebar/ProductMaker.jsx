        ...
        <section className="flex flex-row items-center gap-2">
          <div className="rounded-full border-1 border-gray-200 w-[36px] h-[36px] flex items-center justify-center overflow-hidden">
            ...
            <img
              ...
              src="https://cdn2.wadiz.kr/2023/11/29/c7c008b8-2b79-4ab3-aac2-5f1d659c02f1.jpg/wadiz/resize/100/format/jpg/quality/90/"
            ></img>
          </div>
          <div className="flex flex-col">
            {/* 메이커 이름 */}
            <span className="font-bold">댄꼼마</span>
            {/* 메이커 팔로우 수 */}
            <span className="text-[var(--color-primary)] text-[12px]">
              {" "}
              {Number("29137").toLocaleString("ko-KR")}명이 팔로우 중
            </span>
          </div>
        </section>
        <section>
      ...
      <article className="flex flex-col gap-4 mt-4 text-[14px]">
        <section className="flex flex-row items-center gap-2 justify-between">
          <div className="flex flex-row items-center gap-2">
            ...
            <span>
              만족도 {Number("5.0").toFixed(1)}
              <span className="text-gray-300"> ( {"999+"}개 ) </span>
            </span>
          </div>
        ...
        <section className="flex flex-row items-center gap-2">
          ...
          <span>누적 펀딩액 {"19억원+"}</span>
        </section>
        <section className="flex flex-row items-center gap-2">
          <PiUserFill className="text-[var(--color-primary)]" />
          <span>서포터 {Number("35547").toLocaleString("ko-KR")}명</span>
        </section>
      </article>
    </section>
  );
